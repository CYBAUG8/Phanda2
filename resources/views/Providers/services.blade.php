...
<div x-data="{
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">{{ $showArchived ? 'Archived services for record keeping.' : 'Manage your offerings and pricing.' }}</p>
            <div class="mt-3 inline-flex rounded-lg border border-gray-200 p-1 text-sm">
                <a href="{{ route('provider.services.index') }}"
                   class="rounded-md px-3 py-1.5 {{ !$showArchived ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">
                    Active/Paused
                </a>
                <a href="{{ route('provider.services.index', ['archived' => 1]) }}"
                   class="rounded-md px-3 py-1.5 {{ $showArchived ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">
                    Archived
                </a>
            </div>
    ...
    @if($services->isEmpty())
        <div class="rounded-2xl border border-dashed border-gray-300 bg-white p-12 text-center">
            <p class="text-base font-medium text-gray-800">{{ $showArchived ? 'No archived services' : 'No services yet' }}</p>
            <p class="mt-1 text-sm text-gray-500">{{ $showArchived ? 'Deleted services will appear here.' : 'Add your first service to get started.' }}</p>
            @if(!$showArchived)
                <button @click="addModalOpen = true"
            ...
            @foreach($services as $ps)
                <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $showArchived ? 'bg-gray-200 text-gray-700' : ($ps->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600') }}">
                            {{ $showArchived ? 'Archived' : ($ps->is_active ? 'Active' : 'Paused') }}
                        </span>
                    </div>
                    ...
                    @if(!$showArchived)
                        <div class="mt-4 grid grid-cols-3 gap-2">
                            ...
                            <form action="{{ route('provider.services.destroy', $ps->service_id) }}" method="POST" onsubmit="return confirm('Archive this service offering?')">
                                @csrf
                                @method('DELETE')
                    ...
                    @else
                        <p class="mt-4 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600">
