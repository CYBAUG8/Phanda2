    ...
    <section class="provider-filter-bar space-y-3">
        <div class="flex gap-2 overflow-x-auto whitespace-nowrap pb-1">
            @foreach($statusTabs as $statusKey => $label)
                ...
                @endphp
                <a
                    href="{{ route('provider.bookings.index', $tabQuery) }}"
        ...
        <form action="{{ route('provider.bookings.index') }}" method="GET" class="provider-filter-grid">
            ...
            <div class="provider-filter-actions">
                ...
                <a href="{{ route('provider.bookings.index', ['status' => $activeStatus]) }}" class="ui-btn-secondary w-full justify-center">Reset</a>
            </div>
        </form>
    </section>
